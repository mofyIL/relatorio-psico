...
def classify_by_media(media):
    if pd.isna(media): return 'Sem dados'
    if media<=2.5: return 'Risco Baixo'
    if media<=3.5: return 'Risco Médio'
    return 'Risco Alto'

def color_by_classificacao(clf):
    if clf=='Risco Baixo': return ('70AD47','FFFFFF')
    if clf=='Risco Médio': return ('FFD966','000000')
    if clf=='Risco Alto': return ('C00000','FFFFFF')
...
def get_company_subsets(df_raw,df_scored,company):
    ...
    if sub_raw.empty: raise ValueError(f'Empresa não encontrada: {company}')
    return sub_raw,sub_scored

def get_setores(sub_raw):
    if 'SETOR' not in sub_raw.columns: return ['GERAL']
...
def build_block_results(sub_scored):
    ...
    for bloco in BLOCOS:
        ...
        rows.append({'Bloco':bloco['nome'],'Perguntas':f"{bloco['inicio']} a {bloco['fim']}","Media":round(media,2) if pd.notna(media) else np.nan,'Classificacao':classify_by_media(media)})
...
def build_distribution_table(sub_raw):
    ...
    for bloco in BLOCOS:
        ...
        for i in range(bloco['inicio'],bloco['fim']+1):
            ...
            if col not in sub_raw.columns: continue
...
def get_meta(sub_raw):
    ...
    return {'Setores':setores,'Cargos':cargos}
...
def add_intro(doc,respondents,meta):
    ...
    for k,v in meta.items(): doc.add_paragraph(f'{k}: {v}')
...
def add_distribution_table(doc,dist_df):
    ...
    table.style='Table Grid'
    table.alignment=WD_TABLE_ALIGNMENT.CENTER
    for i,h in enumerate(cols): set_cell_text(table.rows[0].cells[i],h,bold=True,size=8)
...
def add_recommendations(doc,res_blocos):
    ...
    for _,row in res_blocos.iterrows():
        ...
        if clf=='Risco Alto': txt=f'{bloco}: recomenda-se ação imediata, análise das causas, validação com liderança e medidas corretivas formais.'
        elif clf=='Risco Médio': txt=f'{bloco}: recomenda-se monitoramento, revisão de práticas internas e acompanhamento periódico.'
        elif clf=='Risco Baixo': txt=f'{bloco}: manter acompanhamento contínuo e registrar boas práticas preventivas.'
        else: txt=f'{bloco}: revisar dados e preenchimento antes da conclusão final.'
...
def generate_report(company,sector,df_raw,df_scored):
    ...
    for setor in setores:
        ...
        else: setor_raw=sub_raw.copy(); setor_scored=sub_scored.copy()
        if setor_raw.empty: continue
...
def load_from_gsheets_auto():
    """Carrega dados automaticamente dos Secrets do Streamlit"""
    try:
...
def load_from_gsheets_manual(credentials_file, spreadsheet_id):
    """Carrega dados com credentials manual"""
    try:
        from google.oauth2 import service_account
        import googleapiclient.discovery
        import json
...
if not auto_loaded:
    ...
    if modo == "📤 Upload de CSV (manual)":
        ...
        credentials_file = st.file_uploader("Selecione o arquivo credentials.json do Google Cloud", type=["json"])
        spreadsheet_id = st.text_input("ID da Spreadsheet do Google Forms")
        if credentials_file and spreadsheet_id:
            with st.spinner("Conectando..."):
                df_raw, err = load_from_gsheets_manual(credentials_file, spreadsheet_id)
                if df_raw is not None:
