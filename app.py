...
# ================= CONFIGURAÇÕES =================
st.set_page_config(page_title="Gerador de Relatórios Psicossocial", page_icon="📊")
...
BASE_COLS = {
    'CARIMBO DE DATA/HORA', 'CARIMBO DE DATAHORA', 'NOME DA EMPRESA', 'SETOR', 'CARGO',
    'CARGO ATUAL', 'IDADE', 'NOME COMPLETO', 'CPF', 'SEXO', 'PONTUAÇÃO', 'PONTUACAO'
}
...
def convert_answer_to_num(x):
    return MAP.get(norm_text(x), np.nan)
...
def prepare_scored_df(df_raw):
    ...
    for c in df.columns:
        ...
        qnum = next((k for k, v in QUESTIONARIO_NORM.items() if c == v), None)
...
def bloco_columns_present(df, inicio, fim):
    return [QUESTIONARIO_NORM[i] for i in range(inicio, fim + 1) if QUESTIONARIO_NORM[i] in df.columns]
...
def build_distribution_table(sub_raw):
    ...
    for bloco in BLOCOS:
        ...
        for i in range(bloco['inicio'], bloco['fim'] + 1):
            ...
            row = {
                'Bloco': bloco['nome'] if first else '',
                'Numero': i,
                'Pergunta': QUESTIONARIO[i]
            }
            first = False
...
def get_meta(sub_raw):
    setores = ', '.join(sorted({str(x).strip() for x in sub_raw.get('SETOR', pd.Series(dtype=str)).dropna()}))
    cargos_col = 'CARGO ATUAL' if 'CARGO ATUAL' in sub_raw.columns else 'CARGO'
    cargos = ', '.join(sorted({str(x).strip() for x in sub_raw.get(cargos_col, pd.Series(dtype=str)).dropna()}))
    sexos = ', '.join(sorted({str(x).strip() for x in sub_raw.get('SEXO', pd.Series(dtype=str)).dropna()}))
    return {'Setores': setores, 'Cargos': cargos, 'Sexo': sexos}

# ================= FUNÇÕES DE GERAÇÃO DO DOCX =================
def make_chart_blocks(res_blocos, company, sector_dir):
    dfp = res_blocos.copy()
    colors = []
    for clf in dfp['Classificacao']:
        colors.append({'Risco Baixo': '#70AD47', 'Risco Médio': '#FFD966', 'Risco Alto': '#C00000'}.get(clf, '#BFBFBF'))
    ...
    for bar, media, clf in zip(bars, dfp['Media'], dfp['Classificacao']):
        ...
        plt.text(
            ...
            min((media if pd.notna(media) else 0) + 0.08, 4.9),
            label,
            ha='center',
            va='bottom',
            fontsize=9
...
def add_block_table(doc, res_blocos, display_sector):
    ...
    headers = [
        ...
        'CONFLITO INTERPESSOAL E FALTA DE APOIO',
        'RELAÇÕES FÍSICAS E EMOCIONAIS AO TRABALHO'
    ]
    ...
    doc.add_paragraph('')
    doc.add_paragraph('Análise e interpretação dos resultados')
    doc.add_paragraph('• Baixo risco psicossocial: média geral ≤ 2,5')
    doc.add_paragraph('• Risco moderado: média geral entre 2,6 e 3,5')
    doc.add_paragraph('• Risco elevado: média geral > 3,5')
...
def generate_report(company, sector, df_raw, df_scored):
    ...
    for setor in setores:
        ...
        chart_buffer = make_chart_blocks(res_blocos, company, None)
...
# ================= FUNÇÃO DE ACESO AO GOOGLE FORMS =================
def get_google_forms_data(credentials_json, spreadsheet_id):
    """Aciona respostas do Google Forms via Google Sheets API"""
...
# Seção 1: Upload do CSV (modo manual) ou Google Forms (modo automático)
method = st.radio("Como você quer importar os dados?", 
                  ["📤 Upload de CSV (manual)", "🌐 Google Forms (automático)"],
                  index=0)

df_raw = None
df_scored = None

if method == "📤 Upload de CSV (manual)":
    st.subheader("Upload do arquivo CSV")
    uploaded_file = st.file_uploader("Selecione o CSV com as respostas", type=["csv"])
    
    if uploaded_file is not None:
        try:
            df_raw = load_raw_df(uploaded_file)
            df_scored = prepare_scored_df(df_raw)
            st.success(f"✅ CSV importado! {len(df_raw)} respostas encontradas.")
        except Exception as e:
            st.error(f"❌ Erro ao ler CSV: {e}")

elif method == "🌐 Google Forms (automático)":
    st.subheader("Conexão com Google Forms")
    
    credentials_file = st.file_uploader(
        "Selecione o arquivo credentials.json do Google Cloud",
        type=["json"]
    )
    
    spreadsheet_id = st.text_input(
        "ID da Spreadsheet do Google Forms",
        placeholder="Ex: 1BxiMVs0XRA5nFMYM3xZ5B8qK9vL2mN3oP4qR5sT6uV7w"
    )
    
    if credentials_file is not None and spreadsheet_id:
        try:
            # Salva credentials temporariamente
            credentials_path = '/tmp/credentials.json'
            with open(credentials_path, 'wb') as f:
                f.write(credentials_file.read())
            
            df_raw = get_google_forms_data(credentials_path, spreadsheet_id)
            if df_raw is not None:
                df_scored = prepare_scored_df(df_raw)
                st.success(f"✅ Google Forms importado! {len(df_raw)} respostas encontradas.")
            else:
                st.error("❌ Nenhuma resposta encontrada no Google Forms.")
        except Exception as e:
            st.error(f"❌ Erro na conexão: {e}")

# Seção 2: Seleção de empresa e setor
if df_raw is not None and df_scored is not None:
    st.markdown("---")
    ...
    empresas_display = [x for x in empresas]
    
    empresa_selected = st.selectbox("Selecione a empresa:", empresas_display)
    
    # Extrai setores da empresa selecionada
    company_norm = norm_text(empresa_selected)
    mask_company = df_raw['NOME DA EMPRESA'].astype(str).map(norm_text) == company_norm
    sub_raw_company = df_raw[mask_company]
        ...
        with st.spinner("⏳ Gerando relatório..."):
            try:
                ...
                if not buffers:
                    ...
                    st.success(f"✅ Relatório gerado com sucesso! {len(buffers)} setor(es).")
                    ...
                    # Opção de enviar por e-mail (modo simples)
                    st.markdown("---")
                    st.subheader("📧 Enviar por e-mail")
                    email_to = st.text_input("E-mail do destinatário:", placeholder="ex:parceiro@empresa.com")
                    
                    if email_to and st.button("Enviar e-mail", use_container_width=True):
                        st.info("📧 Para enviar e-mail automático, você precisa configurar SMTP no secret.toml. Veja as instruções abaixo.")
                        st.code("""
# No arquivo .streamlit/secret.toml, adicione:
[smtp]
host = "smtp.gmail.com"
port = 587
...
st.markdown("---")
st.markdown("""
### 📋 Instruções para configurar Google Forms (opcional):
...
### 🔐 Para enviar e-mail automático:

Configure o arquivo `.streamlit/secret.toml` com suas credenciais SMTP (Gmail, Outlook, etc).
""")