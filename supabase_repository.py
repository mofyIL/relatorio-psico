...
class SupabaseRepository:
        ...
        response_payload = _clean_dict(
            {
                ...
                "sector": metadata.get("sector") or metadata.get("SETOR"),
                "role_family": metadata.get("role_family") or metadata.get("CARGO"),
                "work_unit": metadata.get("work_unit") or metadata.get("UNIDADE"),
                "demographics": demographics,
                "consent_accepted": _boolish(metadata.get("consent_accepted"), True),
                "dedupe_hash": metadata.get("dedupe_hash"),
                "source": metadata.get("source") or "streamlit",
            }
        )
        response = _first(self.client.table("responses").insert(response_payload).execute())
        if not response:
    ...
    def list_responses_for_campaign(self, campaign_id: str | None = None) -> list[dict[str, Any]]:
        ...
        for response in responses:
            ...
            row: dict[str, Any] = {
                ...
                "SETOR": response.get("sector") or "",
                "CARGO": response.get("role_family") or "",
    ...
    def _campaign_index(self, campaign_ids: list[str]) -> dict[str, dict[str, Any]]:
        ...
        rows = _rows(
            self.client.table("campaigns")
            .select("id,company_id,companies(id,legal_name,trade_name)")
            .in_("id", campaign_ids)
            .execute()
        )
